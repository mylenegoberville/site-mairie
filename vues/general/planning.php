	...
	<body>
		<?php if(isset($_GET["ville"])) { ?>
			...
			<?php for($i = 0; $i < count($liste_batiment); $i++) { ?>
				<p class="nom-batiment"><?php echo ModelPlanning::GetNomBatimentByIdBatiment($liste_batiment[$i]["IdB"])[0][0]; ?></p>
				<?php 
					$_SESSION['idBat'.$liste_batiment[$i]["IdB"]] = $liste_batiment[$i]["IdB"]; 
					echo "<div id=".$liste_batiment[$i]["IdB"]."></div> : ".$_SESSION['idBat'.$liste_batiment[$i]["IdB"]];
				?>
				...
				<table class="table-fill">
					...
					<tbody>
						<?php 
							$liste_salle = ModelPlanning::GetIdSalleByIdBatiment($liste_batiment[$i]["IdB"]);
							for($e = 0; $e < count($liste_salle); $e++) {
						?>
								<tr>
									<th><?php echo ModelPlanning::GetNomSalleByIdSalle($liste_salle[$e]["IdS"])[0][0]; ?></th>
									<?php echo '<td onClick="'; echo "document.location.href='index.php?action=rendezvous&maj=ajout&salle=".$liste_salle[$e]["IdS"]."&batiment=".$_SESSION['idBat'.$liste_batiment[$i]["IdB"]]."&jour=".$id_jour_debut."&nbjour=".ModelPlanning::GetJourByIdDate($id_jour_debut)[0][0]."&mois=".ModelPlanning::GetMoisByIdDate($id_jour_debut)[0][0]."&an=".ModelPlanning::GetAnneeByIdDate($id_jour_debut)[0][0]."&ville=".$ville."'"; echo '"';?>
										<p> 
											<?php
