<?php
	...
	if(isset($_POST['action'])) {
		...
		if($_POST['action']=="delall") {
			...
			$IdAs = ModelPlanning::GetAllInfoManifByIdManif($_POST["IdManif"])[0]['IdAs'];
			$IdS = ModelPlanning::GetAllInfoManifByIdManif($_POST["IdManif"])[0]['IdS'];

			$lesmanifs = ModelPlanning::GetAllManifByAllInfoA($NomM,$HeureDebut,$HeureFin,$Publique,$NombrePersonne,$IdT,$IdAs,$IdS);
	...
	<html>
		<body>
			<?php
				...
				$IdAs = ModelPlanning::GetAllInfoManifByIdManif($_GET["id-manif"])[0]['IdAs'];
